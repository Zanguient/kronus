...
type
  TFrmCadastroTabela = class(TForm)
    ...
    cdsColunasapp_campo_senha: TBooleanField;
    dbchkapp_campo_senha: TDBCheckBox;
    ...
    procedure grdColunasDblClick(Sender: TObject);
    procedure dbedttipo_campoCloseUp(Sender: TObject);
    procedure dbchkapp_campo_senhaClick(Sender: TObject);
    ...
    procedure edtCampoPaiKeyPress(Sender: TObject; var Key: Char);
    procedure btnAddRelacClick(Sender: TObject);
    procedure cdsTabelaDetalheAfterPost(DataSet: TDataSet);
...
begin
  try
    ...
    begin
      ...
      cdsColunas.FieldByName('app_ajax_tabela').FocusControl;
    end;
...
begin
  ...
  begin
    try
      Application.CreateForm(TFrmListaColunas,FrmListaColunas);
      FrmListaColunas.Hint := cdsTabela.FieldByName('nome_tabela').asstring;//cdsColunas.FieldByName('app_ajax_tabela').AsString;
...
begin
  ...
  if cdsColunas.RecordCount = 0 then
   ...
   mr2.Close;
   mr2.ParamByName('nomeTabela').AsString := cdsTabelanome_tabela.AsString;
   mr2.ExecuteMethod;
   existeTabela:=mr2.ParamByName('ReturnParameter').AsBoolean;

  try
    if (Tag = 1) or (existeTabela = false) then //nova tabela
    begin
...
begin
  ...
  cdsColunaschave_primaria.AsBoolean := False;
  cdsColunastipo_campo.AsString := 'varchar';
  cdsColunastamanho_campo.AsInteger := 60;
  cdsColunasauto_incremento.AsBoolean := False;
  cdsColunasaceita_nulo.AsBoolean := True;
  cdsColunasvirtual.AsBoolean := False;
  cdsColunasapp_obrigatorio.AsBoolean := False;
  ...
  cdsColunasapp_titulo_largura.AsInteger := 100;
  cdsColunasguia_nome.AsString := 'Principal';
  cdsColunasapp_campo_senha.AsBoolean := False;
...
procedure TFrmCadastroTabela.cdsTabelaDetalheAfterPost(DataSet: TDataSet);
begin
  limparCamposRelac;
end;
...
procedure TFrmCadastroTabela.dbchkapp_campo_senhaClick(Sender: TObject);
begin
  if cdsColunastipo_campo.AsString <> 'varchar' then
  begin
    dbchkapp_campo_senha.Checked := False;
    cdsColunasapp_campo_senha.AsBoolean := False;
    raise Exception.Create('habilitado semente para campo varchar');
  end;
end;
...
procedure TFrmCadastroTabela.dbedttipo_campoCloseUp(Sender: TObject);
begin
  if cdsColunastipo_campo.AsString = 'int' then
    cdsColunastamanho_campo.AsInteger := 11
  else
  if cdsColunastipo_campo.AsString = 'varchar' then
    cdsColunastamanho_campo.AsInteger := 60
  else
    cdsColunastamanho_campo.AsInteger := 1
...
procedure TFrmCadastroTabela.edtCampoPaiKeyPress(Sender: TObject;
  var Key: Char);
begin
//
  Key := #0;
...
procedure TFrmCadastroTabela.grdColunasDblClick(Sender: TObject);
begin
